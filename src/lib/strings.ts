/**
 * Every string the cashier reads, in one place.
 *
 * Urdu is a future option (spec §6.2). Keeping display text here means
 * localisation is one file swap rather than a hunt through components — so do
 * not hard-code user-facing text in a component.
 *
 * House style: sentence case, and buttons name the action ("Send to kitchen",
 * "Charge"), never "Submit" or "OK".
 */
export const strings = {
  app: {
    name: 'Food Point',
    by: 'Code Hustlers',
  },

  nav: {
    dashboard: 'Dashboard',
    newOrder: 'New order',
    orders: 'Orders',
    tables: 'Tables',
    menu: 'Menu',
    deals: 'Deals',
    reports: 'Reports',
    cancellations: 'Cancellations',
    customers: 'Customers',
    waiters: 'Waiters',
    day: 'Open / close day',
    sales: 'Sales',
    settings: 'Settings',
    expenses: 'Expenses',
  },

  deals: {
    title: 'Deals',
    subtitle: 'Bundles sold at one combo price.',
    addDeal: 'Add deal',
    editDeal: 'Edit deal',
    name: 'Deal name',
    namePlaceholder: 'e.g. Family Deal',
    price: 'Combo price',
    priceHint: 'What the customer pays for the whole bundle.',
    contents: 'What is in it',
    addItem: 'Add an item',
    active: 'On the order screen',
    inactive: 'Off',
    menuValue: 'Parts separately',
    saving: 'Customer saves',
    markup: 'Priced above the parts',
    unavailable: 'Unavailable',
    soldOutBecause: (item: string) => `${item} is sold out`,
    noItemsYet: 'No items in this deal yet.',
    emptyTitle: 'No deals yet',
    emptyNote: 'A deal bundles menu items at one fixed price — Burger + Fries + Drink for Rs 550.',
    deleteConfirm: 'Delete this deal? Past bills keep their record of it.',
    itemsCount: (n: number) => `${n} item${n === 1 ? '' : 's'}`,
    inDeal: 'Deal',
    photo: 'Deal photo',
  },

  staff: {
    title: 'Staff',
    subtitle: 'Who can take orders, and whose name goes on a cancellation.',
    signIn: 'Sign in',
    signOut: 'Sign out',
    switchUser: 'Switch user',
    whoAreYou: 'Who is on the counter?',
    enterPin: 'Enter your PIN',
    pinHint: 'Your own 4-digit PIN, not the manager PIN.',
    signedInAs: 'Signed in',
    addStaff: 'Add staff',
    editStaff: 'Edit staff',
    name: 'Name',
    namePlaceholder: 'e.g. Bilal',
    pin: 'PIN',
    pinSet: 'PIN set',
    noPin: 'No PIN',
    setPin: 'Set a PIN',
    changePin: 'Change PIN',
    pinOptional: 'Leave blank to keep the current PIN.',
    active: 'On the counter',
    inactive: 'Left',
    remove: 'Remove',
    deactivated: 'Removed from the counter — their past cancellations are kept.',
    deleted: 'Removed.',
    emptyTitle: 'No staff added yet',
    emptyNote:
      'Add the people who work the counter. Until you do, cancellations are recorded without a name.',
    signInPrompt: 'Sign in to start taking orders.',
    wrongPin: "That PIN doesn't match.",
  },

  waiters: {
    title: 'Waiters',
    subtitle: 'Who a takeaway order is being run for.',
    addWaiter: 'Add waiter',
    editWaiter: 'Edit waiter',
    name: 'Name',
    namePlaceholder: 'e.g. Ahmed',
    code: 'Code',
    codeOptional: 'optional',
    codePlaceholder: 'e.g. W1',
    active: 'On the dropdown',
    inactive: 'Off',
    remove: 'Remove',
    deleteConfirm:
      'Delete this waiter? Past orders keep their own record of who they were for.',
    deleted: 'Waiter deleted. Past orders are untouched.',
    emptyTitle: 'No waiters added yet',
    emptyNote:
      'Add the people who run takeaway orders. Turn on "Enable waiters" in Settings to ask for one at the till.',
    pay: 'Pay',
    payType: 'Paid',
    wageRate: 'Amount',
    wageRateHint: 'Pre-fills the wages review. Only used when "Enable waiter wages" is on.',
    payday: 'Payday',
    paydayHintWeekly: 'Which day of the week this waiter is paid.',
    paydayHintMonthly: 'Which day of the month this waiter is paid.',
    editPay: 'Edit pay',
    noPay: 'Not set',
    savePay: 'Save pay',
  },

  expenses: {
    title: 'Expenses',
    subtitle: 'Money that left the till today — vegetables, a gas cylinder, and the like.',
    addExpense: 'Add expense',
    description: 'Description',
    descriptionPlaceholder: 'e.g. Vegetables',
    amount: 'Amount',
    total: 'Total expenses',
    netCashPosition: 'Net cash position',
    netCashPositionHint: 'Cash sales minus total expenses — cash in and out of the till, not profit.',
    emptyTitle: 'No expenses recorded today',
    emptyNote: 'Add one below whenever money leaves the till.',
    deleteConfirm: 'Delete this expense?',
    deleted: 'Expense deleted.',
    wagesLineNote: 'Posted automatically from the waiter wages review below — edit it there.',
    recurringLineNote: 'Posted automatically from the recurring-expenses review below — edit it there.',
    disabledNote: 'Both expense toggles are off in Settings, so nothing has ever been recorded here.',
  },

  wages: {
    title: 'Waiter wages',
    subtitle:
      "Today's wages — daily waiters every time, weekly/monthly waiters only on their payday. Pre-filled from each waiter's rate; review and save.",
    rate: 'Amount',
    save: 'Save wages',
    saved: 'Wages saved.',
    total: 'Wages total',
    postedNote: 'Saved as a single "Waiter wages" line in Expenses above.',
    emptyTitle: 'Nobody is due today',
    emptyNote:
      'No active waiter is paid today — daily waiters would appear here, and weekly/monthly waiters appear only on their payday.',
    noWaitersTitle: 'No active waiters',
    noWaitersNote: 'Add waiters in Admin → Waiters to pay them here.',
    disabledNote: '"Enable waiter wages" is off in Settings.',
  },

  recurringExpenses: {
    title: 'Recurring expenses',
    subtitle:
      'Rent, a subscription — anything paid on a standing schedule instead of typed in fresh each time.',
    addRecurring: 'Add recurring',
    editRecurring: 'Edit recurring expense',
    description: 'Description',
    descriptionPlaceholder: 'e.g. Rent',
    amount: 'Amount',
    payType: 'Paid',
    payday: 'Payday',
    paydayHint: 'Which day of the month this is due.',
    active: 'Active',
    inactive: 'Paused',
    emptyTitle: 'No recurring expenses set up',
    emptyNote: 'Add rent, a subscription, or anything else paid on a schedule.',
    deleteConfirm: 'Delete this recurring expense? Past postings keep their own record.',
    deleted: 'Recurring expense deleted.',
    save: 'Save',
    dueTitle: 'Recurring expenses due today',
    dueSubtitle:
      "Daily ones every time, monthly ones only on their payday. Pre-filled from each definition's amount — review and post.",
    post: 'Post expenses',
    posted: 'Recurring expenses posted.',
    postedNote: 'Each posts as its own line in Expenses above.',
    dueEmptyTitle: 'Nothing due today',
    dueEmptyNote:
      'A daily recurring expense would appear here every time; a monthly one appears only on its payday.',
    noneDefinedTitle: 'No recurring expenses set up',
    noneDefinedNote: 'Add one below to have it appear here automatically on its due date.',
  },

  admin: {
    title: 'Admin',
    hint: 'Enter the manager PIN to open the owner’s area.',
    unlock: 'Open admin',
    wrongPin: 'That PIN doesn’t match.',
    backToCounter: 'Back to counter',
    exitAdmin: 'Exit admin',
    adminButton: 'Admin',
    counterArea: 'Counter',
    adminArea: 'Admin',
    setPinTitle: 'Set a manager PIN',
    setPinHint:
      'The admin area holds your takings and settings, so it needs a PIN. This is the same PIN that approves cancelling a paid order.',
    setPinAction: 'Set PIN and continue',
    confirmPin: 'Repeat PIN',
    pinMismatch: 'Those two PINs are not the same.',
    locked: 'Admin locked.',
    forgotPin: 'Forgotten the PIN?',
    recoverTitle: 'Reset the manager PIN',
    recoverHint:
      'Use this computer’s licence key to clear the PIN so you can set a new one. Nothing else is changed. If you do not have the key, send the Machine ID below to the office.',
    recoverAction: 'Verify key and reset PIN',
    machineIdLabel: 'Send this Machine ID to the Code Hustlers office',
    licenceKeyLabel: 'Then paste the licence key they send back',
  },

  theme: {
    dark: 'Dark',
    light: 'Light',
    toLight: 'Switch to the light theme',
    toDark: 'Switch to the dark theme',
  },

  customers: {
    title: 'Customers',
    subtitle: 'Saved by phone number from takeaway and delivery orders.',
    unnamed: 'Saved customer',
    ordersBefore: (n: number) => `${n} order${n === 1 ? '' : 's'} before`,
    search: 'Search name or phone',
    name: 'Name',
    phone: 'Phone',
    address: 'Address',
    notes: 'Notes',
    orders: 'Orders',
    lastOrder: 'Last order',
    never: 'Never',
    edit: 'Edit customer',
    exportCsv: 'Export CSV',
    exported: (n: number) => `${n} customers exported.`,
    deleteConfirm:
      'Delete this customer? Their past orders are kept — only the saved contact is removed.',
    deleted: 'Customer deleted. Past orders are untouched.',
    emptyTitle: 'No customers yet',
    emptyNote:
      'A customer is saved automatically whenever a takeaway or delivery order is placed with a phone number.',
    noResults: 'Nobody matches that search.',
  },

  extras: {
    title: 'Extras',
    subtitle: 'Packaging and disposables, charged on any order type.',
    add: 'Add extra',
    name: 'Name',
    namePlaceholder: 'e.g. Disposable plates',
    price: 'Price',
    cost: 'Cost to you',
    costHint: 'Optional. Keeps profit honest — plates are not free.',
    active: 'Offered',
    inactive: 'Off',
    none: 'No extras set up yet.',
    onOrder: 'Extras',
    addToOrder: 'Add extras',
    emptyOnOrder: 'No extras on this order.',
    setUpFirst: 'Set extras up in Admin → Settings first.',
  },

  images: {
    photo: 'Photo',
    upload: 'Upload photo',
    replace: 'Replace photo',
    pickExisting: 'Pick existing',
    importFolder: 'Import a whole folder',
    remove: 'Remove',
    noPhoto: 'No photo',
    missingFile: 'Photo file missing',
    pickOne: 'Tap a photo to use it',
    folderEmpty: 'No photos uploaded yet.',
    categoryPhoto: 'Category photo',
    itemPhoto: 'Item photo',
  },

  cancel: {
    title: 'Cancel order',
    cancelOrder: 'Cancel order',
    cancelItem: 'Cancel item',
    reason: 'Why is this being cancelled?',
    note: 'What happened?',
    notePlaceholder: 'Say briefly what happened',
    noteRequired: 'Add a short note when the reason is Other.',
    managerPin: 'Manager PIN',
    paidWarning:
      'This order has already been paid. Cancelling it removes the money from today\'s takings and is recorded permanently against your name.',
    paidPinHint: 'A manager or the owner must enter their PIN to cancel a paid order.',
    unpaidNote: 'Nothing has been paid yet. This will be recorded against your name.',
    confirm: 'Cancel this order',
    confirmItem: 'Cancel this item',
    keep: 'Keep it',
    done: 'Cancelled and recorded.',
  },

  cancellations: {
    title: 'Cancellations',
    subtitle: 'Every cancelled order — who, when, why, and how much.',
    today: 'Today',
    week: 'This week',
    month: 'This month',
    totalCount: 'Cancellations',
    totalValue: 'Total cancelled',
    paidCount: 'Paid orders cancelled',
    paidValue: 'Cash cancelled after payment',
    byStaff: 'By staff member',
    byReason: 'By reason',
    theList: 'Every cancellation',
    staffName: 'Staff',
    count: 'Count',
    value: 'Value',
    paid: 'Paid',
    ofWhichPaid: 'of which paid',
    when: 'When',
    order: 'Order',
    reason: 'Reason',
    amount: 'Amount',
    items: 'Items',
    wasPaid: 'Was paid',
    notRecorded: 'Not recorded',
    emptyTitle: 'No cancellations in this period',
    emptyNote: 'Nothing was cancelled. That is the result you want.',
    watchNote:
      'One person well above the others is worth asking about — especially in the paid column.',
  },

  orders: {
    title: 'Orders',
    subtitle: 'Every order that has been paid, voided, or is still owing.',
    all: 'All',
    settled: 'Paid',
    voided: 'Voided',
    unpaid: 'Unpaid',
    unpaidNote:
      'Every order still owing, oldest first — not limited to the dates above, because an order left unpaid on Monday is still unpaid on Friday.',
    unpaidEmptyTitle: 'Nothing is owing',
    unpaidEmptyNote: 'Every order that has been placed has been paid for.',
    owing: 'Owing',
    age: 'Waiting',
    printUnpaid: 'Print unpaid slip',
    unpaidSlipNote:
      'Prints what this order owes, marked UNPAID so it cannot be mistaken for a receipt.',
    goToOrder: 'Open at the till',
    orderNo: 'Order',
    type: 'Type',
    when: 'When',
    items: 'Items',
    payment: 'Payment',
    total: 'Total',
    profit: 'Profit',
    print: 'Print',
    printing: 'Printing…',
    printed: 'Bill sent to the printer.',
    view: 'View',
    viewBill: 'Bill',
    emptyTitle: 'No orders in this range',
    emptyNote: 'Pick a different date range, or take an order to see it here.',
    voidedNote: 'This order was voided and was never paid for.',
    partial: 'Partial',
    recordPayment: 'Record payment',
    recordPaymentTitle: 'Record balance payment',
    recordPaymentHint: 'What the customer is handing over now, towards the balance still owed.',
    paymentRecorded: 'Payment recorded.',
  },

  dashboard: {
    title: 'Today',
    salesToday: "Today's sales",
    profitToday: "Today's profit",
    ordersToday: 'Orders today',
    openOrders: 'Open orders',
    bestSeller: 'Best seller today',
    busiestHour: 'Busiest hour',
    today: 'Today',
    recent: 'Recent orders',
    newOrder: 'New order',
    /**
     * The shop trades 10am to 3am, so naming a date would be a lie by 12:01am.
     * State when the day was OPENED instead — that is the boundary the figures
     * actually use.
     */
    tradingDay: (opened: string) => `Trading day — open since ${opened}`,
    noSalesTitle: 'No orders yet today',
    noSalesNote: 'Takings and profit will appear here as soon as the first order is settled.',
    profitLine: (sales: string, profit: string) => `You sold ${sales} and earned ${profit} profit.`,
  },

  order: {
    newOrder: 'New order',
    dineIn: 'Dine-in',
    takeaway: 'Takeaway',
    delivery: 'Delivery',
    chooseTable: 'Choose a table',
    deliveryAddress: 'Delivery address',
    deliveryAddressHint: 'Required — the rider needs somewhere to go.',
    deliveryAddressPlaceholder: 'House / street / area',
    deliveryCharge: 'Delivery charge',
    phoneLookupHint: 'Type a saved number to fill in the name and address.',
    deliveryNeedsAddress: 'Enter the delivery address before starting the order.',
    waiter: 'Waiter',
    chooseWaiter: 'Choose a waiter',
    waiterNeeded: 'Choose a waiter before starting this order.',
    noActiveWaiters: 'No active waiters — add one in Admin → Waiters.',
    customerName: 'Customer name',
    customerPhone: 'Phone',
    optional: 'optional',
    search: 'Search the menu',
    allItems: 'All',
    itemCount: (n: number) => `${n} item${n === 1 ? '' : 's'}`,
    chooseSize: 'Choose a size',
    chooseSizeHint: 'The price changes with the size.',
    weight: 'Weight',
    weightHint: (unit: string) => `Enter the weight in ${unit}.`,
    from: 'from',
    dealsTab: 'Deals',
    addDeal: 'Add deal to order',
    dealAdded: 'Deal added to the order.',
    sendToKitchen: 'Send to kitchen',
    charge: 'Charge',
    total: 'Total',
    subtotal: 'Subtotal',
    discount: 'Discount',
    serviceCharge: 'Service charge',
    emptyTitle: 'Nothing on this order yet',
    emptyNote: 'Tap an item on the left to add it.',
    soldOut: 'Sold out',
    fired: 'Sent',
    lineNote: 'Note for the kitchen',
    lineNotePlaceholder: 'e.g. less spicy, no onions',
    addToOrder: 'Add to order',
    void: 'Void',
    voidLine: 'Void this item',
    voidOrder: 'Void the whole order',
    voidReason: 'Why is this being voided?',
    voidReasonPlaceholder: 'e.g. customer changed their mind',
    managerPin: 'Manager PIN',
    paymentMethod: 'Payment',
    cash: 'Cash',
    card: 'Card',
    takePayment: 'Take payment',
    fullAmount: 'Full amount',
    partialAmount: 'Take an advance',
    amountReceived: 'Amount received',
    advancePaid: 'Advance paid',
    balanceDue: 'Balance due',
    grandTotal: 'Grand total',
    printBill: 'Print bill',
    reprint: 'Reprint bill',
    settled: 'Paid',
    noOpenOrders: 'No orders are open',
    startOrder: 'Start an order',
    resume: 'Resume',
    switchOrder: 'Switch order',
  },

  tables: {
    title: 'Tables',
    free: 'Free',
    occupied: 'Occupied',
    addTable: 'Add table',
    tableName: 'Table name',
    area: 'Area',
    seats: 'Seats',
    emptyTitle: 'No tables yet',
    emptyNote: 'Add your tables and the floor will show which are free at a glance.',
    disabledTitle: 'Tables are switched off',
    disabledNote: 'Turn dine-in tables on in Settings if this food point seats customers.',
  },

  menu: {
    title: 'Menu',
    addItem: 'Add item',
    sizes: 'Sizes',
    sizesHint: 'Add a row for each size. Leave empty for a single-price item.',
    sizeName: 'Size name',
    addSize: 'Add a size',
    noSizes: 'No sizes — this item has one price.',
    addCategory: 'Add category',
    itemName: 'Item name',
    category: 'Category',
    price: 'Price',
    foodCost: 'Food cost',
    foodCostHint: 'Roughly what the ingredients cost. This is what profit is worked out from.',
    available: 'On the menu',
    soldOut: 'Sold out',
    modifierGroups: 'Options',
    soldByWeight: 'Sold by weight/bulk',
    soldByWeightHint: 'Priced per unit, e.g. rice at Rs 350 per kg. Staff enter the weight when adding it.',
    unitLabel: 'Unit',
    unitLabelPlaceholder: 'e.g. kg',
    emptyTitle: 'Your menu is empty',
    emptyNote: 'Add your first item and it will appear on the order screen straight away.',
    uncategorised: 'Uncategorised',
  },

  reports: {
    title: 'Reports',
    from: 'From',
    to: 'To',
    sales: 'Sales',
    profit: 'Profit',
    orders: 'Orders',
    byType: 'By order type',
    byWaiter: 'By waiter',
    bestSellers: 'Best sellers',
    byHour: 'Sales by hour',
    deliveryTotal: 'Delivery charges',
    extrasTotal: 'Extras & packaging',
    deliveryNote: 'Collected for delivery — no food cost behind it.',
    voids: 'Voids',
    noneTitle: 'Nothing in this range',
    noneNote: 'Pick a different date range to see sales and profit.',
    reason: 'Reason',
    print: 'Print',
  },

  settings: {
    title: 'Settings',
    business: 'Business',
    businessName: 'Business name',
    address: 'Address',
    phone: 'Phone',
    printing: 'Printing',
    customerPrinter: 'Customer bill printer',
    kitchenPrinter: 'Kitchen printer',
    kitchenPrinterHint: 'Leave blank to print kitchen tickets on the customer printer.',
    enableKitchenPrint: 'Print kitchen tickets',
    testPrint: 'Print a test',
    service: 'Charges',
    serviceCharge: 'Service charge %',
    currency: 'Currency symbol',
    dineIn: 'Dine-in',
    enableTables: 'This food point has tables',
    enableTablesHint: 'Turn this off for a pure takeaway counter — the Tables screen disappears.',
    waiters: 'Waiters',
    enableWaiters: 'Enable waiters',
    enableWaitersHint:
      'When on, a takeaway order must name a waiter before it can be started. Manage the list in Admin → Waiters.',
    security: 'Voids',
    managerPin: 'Manager PIN',
    serviceChargeMode: 'Service charge',
    serviceChargeModeHint:
      'A flat amount per bill, or a percentage of it. Added to dine-in bills only — takeaway and delivery are never charged for service.',
    fixedAmount: 'Fixed amount',
    percentage: 'Percentage',
    serviceChargeAmount: 'Amount per bill',
    deliveryCharge: 'Default delivery charge',
    deliveryChargeHint: 'Pre-filled on a delivery order. Staff can change it per order.',
    slip: 'Receipt slip',
    slipHint: 'Printed at the top and bottom of every customer bill.',
    shopLogo: 'Shop logo',
    shopLogoHint: 'Stored on this computer. Never loaded from the internet.',
    receiptFooter: 'Footer line',
    receiptFooterHint: 'Printed under the thank-you. Leave blank for none.',
    receiptFooterPlaceholder: 'e.g. No refund without receipt',
    slipPreview: 'Slip preview',
    slipPreviewNote:
      'Preview only. The logo is not yet printed on the thermal slip — that is being added separately so live printing cannot break. The printed slip currently shows the shop name and details as text.',
    managerPinHint: 'Ask for this PIN before voiding. Leave it blank to allow voids without one.',
    setPin: 'Set PIN',
    changePin: 'Change PIN',
    clearPin: 'Remove PIN',
    backup: 'Backup',
    backupNow: 'Back up now',
    restore: 'Restore from backup',
    lastBackup: 'Last backup',
    neverBackedUp: 'You have never backed up.',
    license: 'License',
    save: 'Save settings',
    saved: 'Settings saved.',

    optionalFeatures: 'Optional features',
    weightItems: 'Weight-based items',
    enableWeightItems: 'Enable weight-based items',
    enableWeightItemsHint:
      'When on, a menu item can be marked "sold by weight" (e.g. rice by the kg) with its own unit and per-unit price. Set this per item in Admin → Menu.',
    partialPayments: 'Partial payments',
    enablePartialPayments: 'Enable partial payments',
    enablePartialPaymentsHint:
      'When on, the charge step can take an advance instead of the full amount. The rest shows as a balance owed until it is collected, from the Orders → Unpaid list.',
    dailyExpenses: 'Daily expenses',
    enableDailyExpenses: 'Enable daily expenses',
    enableDailyExpensesHint:
      'When on, the admin portal grows an Expenses screen for recording cash going out — vegetables, a gas cylinder, and the like. Counter staff never reach it.',
    waiterWages: 'Waiter wages',
    enableWaiterWages: 'Enable waiter wages',
    enableWaiterWagesHint:
      'When on, the Expenses screen grows a daily wages review, pre-filled from each waiter’s rate. Needs waiters — set up the roster in Admin → Waiters first.',
  },

  activation: {
    title: 'Activate Food Point',
    intro:
      'Send this Machine ID to the Code Hustlers office and they will send back a license key for this computer.',
    machineId: 'Machine ID',
    copy: 'Copy',
    copied: 'Copied',
    keyLabel: 'License key',
    keyPlaceholder: 'CH1...',
    activate: 'Activate',
    activating: 'Checking…',
    success: 'Activated. Opening the app…',
  },

  /**
   * Open Day / Close Day.
   *
   * "Day" here means the trading session, not the calendar date — the shop
   * serves past midnight, so the wording avoids "today" everywhere.
   */
  day: {
    title: 'Open / close day',
    subtitle: 'Start the trading day, and close it with a report of what was taken.',
    openDay: 'Open day',
    closeDay: 'Close day',
    dayOpen: 'Day open since',
    noDayOpen: 'No day is open',
    noDayOpenNote:
      'Orders can still be taken, but they will not appear on any day report until a day is opened.',
    float: 'Opening cash',
    floatHint: 'What is in the drawer before trading starts. Leave blank if you do not count it.',
    unpaidNow: 'Unpaid so far',
    past: 'Previous days',
    noHistory: 'No days have been closed yet',
    opened: 'Opened',
    closed: 'Closed',
    stillOpen: 'Open now',
    viewReport: 'Report',
    print: 'Print',
    printed: 'Report sent to the printer.',
    opened_: 'Day opened.',
    closed_: 'Day closed. Report sent to the printer.',
    closeAndPrint: 'Close day and print',
    closeAnyway: 'Close day anyway',
    closeHint: 'The day will be closed and the end-of-day report printed.',
    unpaidWarning: (count: number, total: string) =>
      `${count} order${count === 1 ? '' : 's'} worth ${total} ${count === 1 ? 'has' : 'have'} not been paid. ` +
      'Closing now leaves that money off the report. Settle them first if you can.',
    report: 'End of day report',
    sales: 'Sales',
    grossProfit: 'Gross profit (sales − item cost)',
    orders: 'Orders',
    cancelled: 'Cancelled',
    serviceCharges: 'Service charges',
    /* The order-list tab and its two ways out of the app. */
    tabSummary: 'Summary',
    tabOrders: 'Orders',
    ordersOnDay: 'Every order taken between opening and closing.',
    noOrdersOnDay: 'No orders were taken on this day.',
    savePdf: 'Save as PDF',
    printSheet: 'Print',
    printSlip: 'Print slip',
    sheetSaved: (where: string) => `Saved to ${where}`,
    sheetPrinted: 'Sent to the printer.',
    colOrder: 'Order',
    colTime: 'Time',
    colType: 'Type',
    colWho: 'Table / customer',
    colItems: 'Items',
    colPaid: 'Paid',
    colTotal: 'Total',
    statusCancelled: 'Cancelled',
    statusUnpaid: 'UNPAID',
    deliveryCharges: 'Delivery charges',
    cash: 'Cash',
    card: 'Card',
    expectedCash: 'Cash expected in drawer',
    unpaidAtClose: 'Unpaid at close',
    partiallyPaid: 'Partially paid — balance owed',
    topItems: 'Best sellers',
  },

  appearance: {
    title: 'Appearance',
    hint:
      'The colours and artwork below apply to every screen in the app — the ' +
      'sidebar, the order screen, badges, charts and the dashboard.',
    preset: 'Ready-made themes',
    presetHint: 'Start from one of these, then adjust anything you want.',
    custom: 'Your own',
    colours: 'Brand colours',
    coloursHint:
      'Nine colours drive the whole app. Everything else is worked out from them.',
    warningTitle: 'Worth checking before you save:',
    artwork: 'Background artwork',
    artworkHint:
      'Your own image, stored on this computer. Never loaded from the internet.',
    artOpacity: 'Strength on the dashboard',
    artOpacityHint: 'The dashboard is looked at. Higher shows more of the image.',
    artVeil: 'Held back on working screens',
    artVeilHint:
      'The order screen and reports are read all day, so the artwork sits ' +
      'further back there. Higher hides more of it.',
    liveHint: 'Changes preview live. Save to keep them.',
    reset: 'Reset to the standard look',
  },

  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    close: 'Close',
    confirm: 'Confirm',
    loading: 'Loading…',
    search: 'Search',
    none: '—',
    yes: 'Yes',
    no: 'No',
  },
};
